				...
				<div style="border: 1px black solid;height: 500px;">
				  ...
				  <c:if test="${page.list!=null}">
				          ...
				          <c:forEach varStatus="s" var="file" items="${page.list}">
				            ...
				            <a href="${pageContext.request.contextPath}/changeFile_changeFile.do?fileId=${file.id}">修改</a>					         
				            </th>
				          </tr>
				          </c:forEach>
				          </table>
				             <c:if test="${0<page.pageNum-1}">
				             <a href="${pageContext.request.contextPath}/changeFile_execute.do?pageNum=${page.pageNum-1}">上一页</a>
				             </c:if>
				            
				             >>第${page.pageNum}页<< 
				           
				             <c:if test="${page.totalPage>=page.pageNum+1}">
				             <a href="${pageContext.request.contextPath}/changeFile_execute.do?pageNum=${page.pageNum+1}">下一页</a>
				             </c:if>
				   </c:if>
				    <c:if test="${page.list==null}">
